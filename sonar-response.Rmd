...
Pout_tbl <- fluke_freq_tbl %>% 
  ...
  mutate(CLpref = CLpref_fun(Mass),
         CLmax = CLmax_fun(Mass),
         Pout = Pout_fun(Mass, FlukeFreq))
...
TODO: finish functional response section and get good estimates for *t~d~* and *t~f~*. For now, let's say Mesoplodon and Ziphius are both displaced for 30 minutes and flee for 10 minutes.
...
E_sonar_tbl <- functional_responses %>% 
  ...
  mutate(E_in_J = Pin * t_d_min * 60,
         E_out_J = Pout * t_f_min * 60,
         E_sonar_J = E_in_J + E_out_J)

E_sonar_tbl %>% 
  mutate(binom = abbr_binom(binomial)) %>% 
  gather(E, val, E_in_J, E_out_J) %>% 
