...
Ep_tbl <- prey_data %>%
  ...
  summarize(Ep_kJ = mean(`Energy (kJ)`),
            EpH_kJ = Ep_kJ + sd(`Energy (kJ)`),
            EpL_kJ = Ep_kJ - sd(`Energy (kJ)`))
...
rf_tbl <- rorqual_data %>% 
  ...
  rbind(odontocete_data %>%  
          group_by(binomial) %>% 
          summarize(rf_h = round(mean(total_buzz_count / total_duration_h), 1)))

Pin_tbl <- inner_join(Ep_tbl, rf_tbl, by = "binomial") %>% 
  mutate(Pin_kJ_h = Ep_kJ * rf_h,
         PinL_kJ_h = EpL_kJ * rf_h,
         PinH_kJ_h = EpH_kJ * rf_h)

Pin_tbl %>% 
  mutate(binomial = str_replace(binomial, " ", "\n")) %>% 
  ggplot(aes(x = binomial)) +
  geom_col(aes(y = Pin_kJ_h)) +
  geom_errorbar(aes(ymin = PinL_kJ_h,
                    ymax = PinH_kJ_h),
                width = 0.25) +
  scale_y_log10(name = "Energy (kJ)",
                labels = comma) +
...
Fluking frequency in odontocetes scales with length as $log_{10}(f)=-1.686 \times log_{10}(m) + 1.062$, but not a lot of variance explained (*r^2^=`r sprintf("%.2f", odontocete_model$rsquare)`*). Data from Frank Fish via Will G.

```{r fig.width=5, fig.height=3.5}
ggplot(odontocete_fluking, aes(log10(`Length (m)`), log10(`Frequency (Hz)`))) +
...
Fluking frequency in mysticetes scales with length as $log_{10}(f)=-0.529 \times log_{10}(m) - 0.038$. These are Will's data. Much more variance explained than for odontocetes (*r^2^=`r sprintf("%.2f", mysticete_model$rsquare)`*)
...
mysticete_fluking %>% 
  mutate(abbr = paste(str_sub(binomial, 1, 1), 
                      ...
                      sep = ".")) %>% 
  ggplot(aes(log10(`Length`), log10(`Tailbeat Frequency (Hz)`))) +
...
odontocete_line <- odontocete_fluking %>% 
  summarize(L1 = log10(min(`Length (m)`)),
            L2 = log10(max(`Length (m)`))) %>% 
  mutate(F1 = L1 * odontocete_coef$Slope + odontocete_coef$Intercept,
         F2 = L2 * odontocete_coef$Slope + odontocete_coef$Intercept,
         clade = "odontocete")
mysticete_line <- mysticete_fluking %>% 
  summarize(L1 = log10(min(Length)),
            L2 = log10(max(Length))) %>% 
  mutate(F1 = L1 * mysticete_coef$Slope + mysticete_coef$Intercept,
         F2 = L2 * mysticete_coef$Slope + mysticete_coef$Intercept,
         clade = "mysticete")
...
transmute(odontocete_fluking,
  ...
  rbind(transmute(mysticete_fluking,
                  ...
                  abbr = paste(str_sub(binomial, 1, 1), 
                               ...
                               sep = "."),
                  clade = "Mysticete")) %>% 
  ggplot(aes(log10(length), log10(freq))) +
...
morphologies <- read_csv("data/foragestats_combined_ko2.csv") %>% 
  ...
  mutate(BMR_kcal_day = 70 * Mass ^ 0.75,
...
fluke_freq_tbl <- morphologies %>% 
  ...
  mutate(clade = ifelse(Species == "Balaenoptera musculus",
                        "M", "O"),
         fluke_freq_Hz = scale_fluke_freq(clade, Length)) %>%
  select(-clade)

fluke_freq_tbl %>% 
  mutate(fluke_freq_Hz = round(fluke_freq_Hz, 3)) %>%
  rename(`Length (m)` = Length,
         `Fluking frequency (Hz)` = fluke_freq_Hz) %>% 
  kable("latex", booktabs = TRUE)
```

Given the equations for $P_{out}$, $C_{L.pref}$, and $C_{L.max}$, the estimates for power expended from increased fluking are:

```{r, results="asis"}
CLpref_fun <- function(m) 1.46 + 0.0005 * m
CLmax_fun <- function(m) 5.17 + 0.0002 * m
Pout_fun <- function(m, f) f * m * (CLmax_fun(m) - CLpref_fun(m))

Pout_tbl <- fluke_freq_tbl %>% 
  left_join(select(morphologies, Species, Mass), by = "Species") %>% 
  rename(binomial = Species) %>% 
  mutate(CLpref_J = CLpref_fun(Mass),
         CLmax_J = CLmax_fun(Mass),
         Pout_W = Pout_fun(Mass, fluke_freq_Hz))

Pout_tbl %>% 
  mutate(Species = paste(str_sub(binomial, 1, 1), 
                         str_extract(binomial, " .*"), 
                         sep = ".")) %>% 
  ...
  rename(`Mass (kg)` = Mass,
         ...
         `$C_{L.pref}$` = CLpref_J,
         `$C_{L.max}$` = CLmax_J,
         `$f_f (Hz)$` = fluke_freq_Hz,
         `$P_{out} (W)$` = Pout_W) %>% 
  kable("latex", booktabs = TRUE, escape = FALSE,
        digits = c(0, 1, 3, 0, 2, 2, 0)) %>% 
  footnote("$C_L$ is in units of $J kg^{-1} stroke^{-1}$",
           escape = FALSE)
```

This shows the scaling relationship between cost of locomotion and body length derived for odontocetes in Williams et al. 2017 cannot be applied to an animal the size of a blue whale. Furthermore, I don't think the *Berardius* fluke frequency estimate is reliable because 1) *Berardius* is 69% longer than the longest odontocete in the fluking data (*O. orca*, 6.22 m) and 2) the mysticete fluking model wasn't the strongest predictor (*r^2^=`r sprintf("%.2f", odontocete_model$rsquare)`*).

# Modeled energy costs of sonar exposure
## Beaked whales

TODO: finish functional response section and get good estimates for *t~d~* and *t~f~*. For now, let's say Mesoplodon and Ziphius are both displaced for 30 minutes and flee for 10 minutes. This figure isn't meaningful, it's simply to demonstrate the calculations work.

```{r}
functional_responses <- tibble(binomial = c("Mesoplodon densirostris",
                                            "Ziphius cavirostris"),
                               t_d_min = c(6*60, 6*60),
                               t_f_min = c(30, 30))

E_sonar_tbl <- functional_responses %>% 
  left_join(Pin_tbl, by = "binomial") %>% 
  left_join(Pout_tbl, by = "binomial") %>% 
...
E_sonar_tbl %>% 
  mutate(binom = abbr_binom(binomial)) %>% 
  gather(E, val, E_in_kJ, E_out_kJ) %>% 
  mutate(E = recode(E, 
                    E_in_kJ = "Energy intake",
                    E_out_kJ = "Energy output")) %>% 
  ggplot(aes(binom, val, fill = E)) +
  geom_col() +
