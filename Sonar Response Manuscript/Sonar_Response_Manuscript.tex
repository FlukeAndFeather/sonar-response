...
Across body sizes, \(P_{out}\) increased steeply with flight speed until
an inflection point at approximately 1.8 m/s (Fig. \ref{Pout_fig}).
...
\caption{Predicted stroke frequency (\(f_s\)) at crusing speed.
\label{fs_fig}}
\end{figure}
...
\textit{B. musculus} (blue) and \textit{Z. cavistrostris} (red).
\label{Pout_fig}}
\end{figure}
