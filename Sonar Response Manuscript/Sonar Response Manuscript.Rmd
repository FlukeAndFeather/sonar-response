...
esonar_tbl <- tibble(binomial = factor(c("Mesoplodon densirostris",
                                         "Ziphius cavirostris",
...
# Save a version for presentations
Ep_fig_big <- Ep_fig +
  labs(y = expression(E[p] ~~ (kJ))) +
...
ggsave("Presentation_figs/Ep_fig.pdf", Ep_fig_big, width = 9.5, height = 4.76)
...
rf_fig <- rorqual_data %>% 
  ...
  mutate(binomial = fct_relabel(binomial, abbr_binom)) %>% 
  ggplot(aes(binomial, rf_h, color = Group)) +
  geom_boxplot() + 
  scale_color_aaas() +
  labs(y = expression(r[f] * " (per hour)")) +
  theme_classic(base_size = 18) +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(hjust = 1,
                                   angle = 30,
        ...
        legend.text = element_text(size = 14))
ggsave("Presentation_figs/rf_fig.pdf", rf_fig, width = 9.5, height = 4.76)
```
...
Pin_fig_big <- Pin_fig +
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.direction = "horizontal",
        legend.text = element_text(size = 14))
ggsave("Presentation_figs/Pin_fig.pdf", Pin_fig, width = 9.5, height = 4.76)
...
# For presentations only
# Mass-specific
esonar_fig1 <- esonar_tbl %>% 
  mutate(binomial = fct_relabel(binomial, abbr_binom),
         E_in_kJ2 = E_in_kJ / Mass_kg,
         E_out_kJ2 = E_out_kJ / Mass_kg) %>% 
  gather(direction, energy, E_in_kJ2:E_out_kJ2) %>% 
  ggplot(aes(binomial, energy, fill = direction)) +
  geom_col() +
  scale_fill_aaas(labels = expression(E[`in`], E[out])) +
  labs(y = expression("Mass-specific " * E[sonar] ~~ (kJ ~~ kg^-1))) +
  theme_classic(base_size = 18) +
  theme(legend.position = c(0.15, 0.8),
        legend.title = element_blank(),
        legend.text = element_text(hjust = 0, margin = margin(0, 0, 0, 5)),
        axis.title.x = element_blank(),
        axis.text.x = element_text(face = "italic"))

# BMR ratio
esonar_fig2 <- esonar_tbl %>% 
  mutate(binomial = fct_relabel(binomial, abbr_binom)) %>% 
  ggplot(aes(binomial, E_BMR)) +
  geom_col() +
  scale_y_continuous(limits = c(0, 10),
                     breaks = seq(0, 10, by = 2.5)) +
  labs(y = expression(E[sonar] : BMR)) +
  theme_classic(base_size = 18) +
  theme(legend.position = c(0.15, 0.8),
        legend.title = element_blank(),
        legend.text = element_text(hjust = 0, margin = margin(0, 0, 0, 5)),
...
esonar_fig <- ggpubr::ggarrange(esonar_fig1, esonar_fig2, ncol = 2)
ggsave("Presentation_figs/Esonar_fig.pdf", esonar_fig, width = 13, height = 4.76)
```
